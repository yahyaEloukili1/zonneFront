...
export class Zonne3Component implements OnInit {
  ...
  showInput(){
    this.zoneService.getResource2(this.zoneService.host+"/affectations/search/findByCinContainsIgnoreCaseAndType2Id?mc="+this.inputValue+"&id="+3).subscribe(data=> {
...
this.zoneService.getOneResource(`${this.zoneService.host}`+`/affectations/`+id).subscribe(data=>{
  ...
  onTypeChange2(ev){
    ...
    this.zoneService.getResource2(this.zoneService.host+"/affectations/search/findByStockIdAndType2Id?id="+this.typeSelected2+"&id2="+3).subscribe(data=>{
      this.affectations = data
